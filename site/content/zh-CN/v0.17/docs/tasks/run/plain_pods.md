...
本指南适用于对 Kueue 有基本了解的[批处理用户](/zh-cn/v0.17/docs/tasks#batch-user)。更多信息请参见 [Kueue 概述](/zh-cn/v0.17/docs/overview)。
...
2. 对于 Kueue v0.15 及更早版本，
   学习如何[使用自定义管理器配置安装 Kueue](/zh-cn/v0.17/docs/installation/#install-a-custom-configured-released-version)
   并启用 `pod` 集成。
...
3. 如果启用了 Pod 集成，Kueue 将为所有创建的 Pod 运行 Webhook。Webhook namespaceSelector
   ...
   当您[通过 Helm 安装 Kueue](/zh-cn/v0.17/docs/installation/#install-via-helm) 时，Webhook
...
5. 查看[管理集群配额](/zh-cn/v0.17/docs/tasks/manage/administer_cluster_quotas)了解初始 Kueue 设置的详细信息。
...
目标[本地队列](/zh-cn/v0.17/docs/concepts/local_queue)应在 Pod 配置的 `metadata.labels` 部分中指定。
...
JobSet、MPIJob、RayJob（更多信息请参见[这里](/zh-cn/v0.17/docs/tasks/#batch-user)）。
{{% /alert %}}
