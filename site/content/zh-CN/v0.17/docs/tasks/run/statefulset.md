...
本指南适用于[服务用户](/zh-cn/v0.17/docs/tasks#serving-user)他们具有基本的 Kueue 理解。
更多信息，请参见 [Kueue 概览](/zh-cn/v0.17/docs/overview)。
...
2. 对于 Kueue v0.15 及更早版本，
   学习如何[安装 Kueue 并配置自定义管理器版本](/zh-cn/v0.17/docs/installation/#install-a-custom-configured-released-version)，
   ...
   integrations:
     frameworks:
      ...
      - "statefulset"
   ```
   同时，请参见[运行 Plain Pod](/zh-cn/v0.17/docs/tasks/run/plain_pods/#before-you-begin)
   了解如何启用和配置 `pod` 集成。

3. 检查[管理集群配额](/zh-cn/v0.17/docs/tasks/manage/administer_cluster_quotas)了解初始 Kueue 设置的详细信息。
...
目标[本地队列](/zh-cn/v0.17/docs/concepts/local_queue)应在 StatefulSet 配置的 `metadata.labels` 部分指定。
