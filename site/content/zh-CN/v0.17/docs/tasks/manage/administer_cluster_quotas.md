...
本页面的目标读者是[批处理管理员](/zh-cn/v0.17/docs/tasks#batch-administrator)。
...
- [已安装 Kueue](/zh-cn/v0.17/docs/installation)。

## 安装单一 ClusterQueue 和 ResourceFlavor {#single-clusterqueue-and-single-resourceflavor-setup}

在以下步骤中，你将创建一个排队系统用于管理集群的配额，它拥有
一个 ClusterQueue 和一个 [ResourceFlavor](/zh-cn/v0.17/docs/concepts/cluster_queue#resourceflavor-object)。
...
### 1. 创建 [ClusterQueue](/zh-cn/v0.17/docs/concepts/cluster_queue) {#1-create-clusterqueue}
...
`cpu` 和 `memory` 的使用。每种资源类型都有一个名为 `default` 的 [ResourceFlavor](/zh-cn/v0.17/docs/concepts/cluster_queue#resourceflavor-object)，
并设置了名义配额。

空的 `namespaceSelector` 允许任何命名空间使用这些资源。

### 2. 创建 [ResourceFlavor](/zh-cn/v0.17/docs/concepts/cluster_queue#resourceflavor-object) {#2-create-resourceflavor}
...
### 3. 创建 [LocalQueues](/zh-cn/v0.17/docs/concepts/local_queue) {#3-create-localqueues}

用户不能直接将[工作负载](/zh-cn/v0.17/docs/concepts/workload)
发送到 ClusterQueue。
...
你可以为不同的 [ResourceFlavor](/zh-cn/v0.17/docs/concepts/cluster_queue#resourceflavor-object)定义配额。
...
两个或多个 ClusterQueue 可以在同一 [cohort](/zh-cn/v0.17/docs/concepts/cluster_queue#cohort)
中相互借用未使用的配额。
...
注意，ClusterQueue `team-a-cq` 还定义了 [borrowingLimit](/zh-cn/v0.17/docs/concepts/cluster_queue#borrowingLimit)。
...
也可以从 [cohort](/zh-cn/v0.17/docs/concepts/cluster_queue#cohort)
...
请按照[使用自定义配置的安装说明](/zh-cn/v0.17/docs/installation#install-a-custom-configured-released-version)
操作，并在配置中添加如下字段：
