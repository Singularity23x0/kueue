...
这是全有或全无调度的简单实现。
本页面面向[批处理管理员](/zh-cn/v0.17/docs/tasks#batch-administrator)。
...
- [已安装 Kueue](/zh-cn/v0.17/docs/installation)，版本为 0.3.0 或更高。

## 启用 waitForPodsReady {#enabling-waitforpodsready}

请按照[此处](/zh-cn/v0.17/docs/installation#install-a-custom-configured-released-version)的说明，
使用如下字段扩展配置来安装某个发布版本：
...
一旦重新入队次数达到上限，Kueue 会[停用该 Workload](/zh-cn/v0.17/docs/concepts/workload/#active)。
