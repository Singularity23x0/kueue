...
并发准入（Concurrent Admission）允许 Kueue 为同一个[工作负载](/v0.17/docs/concepts/workload)
保留多个准入尝试，每个准入尝试都被限制到不同的
[ResourceFlavor](/v0.17/docs/concepts/resource_flavor)。这样，工作负载可以先在已准入的规格上启动，
同时 Kueue 继续尝试更优先的规格。
...
- [设置并发准入](/v0.17/docs/tasks/manage/setup_concurrent_admission)。
- 了解 [ClusterQueue 规格顺序](/v0.17/docs/concepts/cluster_queue#resource-flavors-and-resources)。
- 阅读[工作负载概念](/v0.17/docs/concepts/workload)，了解 Parent Workload 和 Variant Workload。
- 阅读 [`ConcurrentAdmissionPolicy` API 参考](/v0.17/docs/reference/kueue.v1beta2/#kueue-x-k8s-io-v1beta2-ConcurrentAdmissionPolicy)。
