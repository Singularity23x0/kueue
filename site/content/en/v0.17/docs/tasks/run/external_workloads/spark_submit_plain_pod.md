...
[SparkApplication integration](/v0.17/docs/tasks/run/kubeflow/sparkapplications/) instead.

When using `spark-submit` in cluster mode, Spark creates a driver Pod that in turn creates the
executor Pods. Kueue can manage these Pods through the [Plain Pod](/v0.17/docs/tasks/run/plain_pods)
...
1. Follow the steps in [Run Plain Pods](/v0.17/docs/tasks/run/plain_pods/#before-you-begin) to learn how to enable and configure the `pod` integration.
...
- the `kueue.x-k8s.io/queue-name` label, to select the target [local queue](/v0.17/docs/concepts/local_queue);
